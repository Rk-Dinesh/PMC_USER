    ...
    <div className="mx-4 my-6 font-poppins">
      ...
      {ticket.map((ticket,index)=>(
        ...
        <div className="flex flex-col gap-1">
            ...
            <p>Status : <span className={` px-3 rounded-md ${ticket.status === 'New Ticket' ? 'bg-green-700': ticket.status === 'In Progress' ? 'bg-yellow-500': ticket.status === 'On Hold' ? 'bg-red-400' : 'bg-blue-400' }`}>{ticket.status}</span></p>
        </div>
        <div>
        <button
